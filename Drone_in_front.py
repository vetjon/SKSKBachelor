...
class Drone:
    def __init__(self, angle_offset, radius):
        self.angle_offset = angle_offset  # Offset in degrees relative to the boat's heading
        self.radius = radius
    ...
    def move(self, boat_x, boat_y, boat_heading):
        ...
        relative_angle = boat_heading + self.angle_offset  # Adjust offset to the heading
        angle = math.radians(relative_angle % 360)  # Ensure the angle is within 0-360 degrees
...
def reset_game():
    ...
    drones = [
        Drone(angle_offset, SCREEN_RADIUS)
        for angle_offset in np.linspace(-45, 45, NUM_DRONES, endpoint=True)
    ]
...
while running:
    ...
    submarine.move_towards_boat(boat, SUB_SONAR_RANGE)
